...
token random_partitioner::get_token_impl(bytes data) {
    md5_hasher h;
...
token random_partitioner::get_token(const schema& s, partition_key_view key) {
    ...
    return get_token_impl(bytes(legacy.begin(), legacy.end()));
}
...
token random_partitioner::get_token(const sstables::key_view& key) {
    ...
    return get_token_impl(bytes(v.begin(), v.end()));
}

token random_partitioner::get_token(const bytes& key) {
    auto v = bytes_view(key);
    if (v.empty()) {
        return minimum_token();
    }
