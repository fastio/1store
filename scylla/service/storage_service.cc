...
future<> storage_service::start_redis_transport() {
    return run_with_api_lock(sstring("start_redis_transport"), [] (storage_service& ss) {
        ...
        auto rserver = make_shared<distributed<cql_transport::redis_server>>();
        ss._redis_server = rserver;

        auto& cfg = ss._db.local().get_config();
        auto addr = cfg.redis_rpc_address();
        auto ceo = cfg.client_encryption_options();
        auto keepalive = cfg.rpc_keepalive();
        return seastar::net::dns::resolve_name(addr).then([rserver, addr, &cfg, keepalive, ceo = std::move(ceo)] (seastar::net::inet_address ip) {
            return rserver->start(std::ref(service::get_redis_storage_proxy())).then([rserver, &cfg, addr, ip, ceo, keepalive]() {
                auto f = make_ready_future();
                struct listen_cfg {
                    ipv4_addr addr;
                    std::shared_ptr<seastar::tls::credentials_builder> cred;
                ...
                std::vector<listen_cfg> configs({ { ipv4_addr{ip, cfg.native_redis_transport_port()} }});
                ...
                if (ceo.at("enabled") == "true") {
                    ...
                    if (cfg.native_redis_transport_port_ssl.is_set() && cfg.native_redis_transport_port_ssl() != cfg.native_redis_transport_port()) {
                        configs.emplace_back(listen_cfg{ipv4_addr{ip, cfg.native_redis_transport_port_ssl()}, std::move(cred)});
                    } else {
                        configs.back().cred = std::move(cred);
                    }
                }
                ...
                return f.then([rserver, configs = std::move(configs), keepalive] {
                    return parallel_for_each(configs, [rserver, keepalive](const listen_cfg & cfg) {
                        return rserver->invoke_on_all(&cql_transport::redis_server::listen, cfg.addr, cfg.cred, keepalive).then([cfg] {
