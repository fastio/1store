...
/*
// Wrapper for REDIS_INTERNAL_DATA_BUS
void messaging_service::register_redis_internal_data_bus(
        std::function<future<redis::result>(std::function< future<redis::result> (const rpc::client_info&, redis::request_wrapper&)> && func)>) {
     register_handler(this, messaging_verb::REDIS_INTERNAL_DATA_BUS, std::move(func));
}
void messaging_service::unregister_redis_internal_data_bus() {
    _rpc->unregister_handler(messaging_verb::REDIS_INTERNAL_DATA_BUS);
}
future<redis::result> messaging_service::edis_internal_data_bus(msg_addr id, redis::request_wrapper&& req) {
   return send_message<redis::result>(this, messaging_verb::REDIS_INTERNAL_DATA_BUS, std::move(id), std::move(req));
}
*/
} // namespace net
