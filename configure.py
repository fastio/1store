...
peids_tests_dependencies = pedis_core + api + idls + [
]

pedis_tests_seastar_deps = [
]


deps = {
    'pedis': idls + ['main.cc'] + pedis_core + api,
}
...
for t in tests_not_using_seastar_test_framework:
    if not t in tests:
        raise Exception("Test %s not found in pedis_tests" % (t))

for t in tests:
    deps[t] = [t + '.cc']
    if t not in tests_not_using_seastar_test_framework:
        deps[t] += pedis_tests_dependencies 
        deps[t] += pedis_tests_seastar_deps
    else:
        deps[t] += pedis_core + api + idls
...
with open(buildfile, 'w') as f:
    ...
    for mode in build_modes:
        modeval = modes[mode]
        f.write(textwrap.dedent('''\
            cxxflags_{mode} = -I. -I./utils -I $builddir/{mode}/gen -I seastar -I seastar/build/{mode}/gen
            rule cxx.{mode}
