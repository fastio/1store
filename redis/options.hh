...
public:
    ...
    explicit redis_options(const db::consistency_level rcl, const db::consistency_level wcl, const timeout_config& tc, auth::service& auth, const socket_address addr)
        :_ks_name("REDIS_0")
    ...
    explicit redis_options(const sstring& ks_name, const db::consistency_level rcl, const db::consistency_level wcl, const timeout_config& tc, auth::service& auth, const socket_address addr)
        :_ks_name(ks_name)
        ,_read_consistency(rcl)
        ,_write_consistency(wcl)
        ,_timeout_config(tc)
