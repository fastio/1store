...
redis_server::connection::connection(redis_server& server, socket_address server_addr, connected_socket&& fd, socket_address addr)
    ...
    , _options(server._config._read_consistency_level, server._config._write_consistency_level, server._config._timeout_config, server._auth_service, addr)
{
...
future<> redis_server::connection::process()
{
    return do_until([this] {
        ...
        logging.info(" std::exception = {} ", e.what());
...
future<> redis_server::connection::process_request() {
    ...
    return _read_buf.consume(_parser).then([this] {
        ...
        return process_request_internal().then([this, leave = std::move(leave)] (auto&& result) mutable {
            --_server._requests_serving;
