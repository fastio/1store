                ...
                return with_linearized_managed_bytes([&] {
                    try {
                        auto evict_last = [this](lru_type& lru) {
                            ...
                            auto it = this_type::cache_type::s_iterator_to(ce);
...
public:
    ...
    using cache_type = bi::set<cache_entry_type,
...
private:
    ...
    cache_type::iterator partitions_end() {
...
public:
    ...
    cache_type::iterator end() const {
        return _cache.end();
    }

    cache_type::iterator find(const Key& key) {
    ...
    void clear_now() noexcept {
        with_allocator(_tracker.allocator(), [this] {
            auto it = _cache.erase_and_dispose(_cache.begin(), cache_end(), [this, deleter = current_deleter<cache_entry_type>()] (auto&& p) mutable {
                //_tracker.on_erase();
                deleter(p);
            });
        });
