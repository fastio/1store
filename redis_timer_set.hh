...
public:
    ...
    using timer_list_t = std::list<lw_shared_ptr<Timer>>; 
private:
...
private:
    ...
    static timestamp_t get_timestamp(lw_shared_ptr<Timer>& timer)
    {
    ...
    int get_index(lw_shared_ptr<Timer>& timer) const
    {
...
public:
     ...
     */
    bool insert(lw_shared_ptr<Timer> timer)
    {
     ...
     */
    void remove(lw_shared_ptr<Timer> timer)
    {
        auto index = get_index(timer);
        auto& list = _buckets[index];
        auto r = std::find(list.begin(), list.end(), timer);
        list.erase(r);
        if (list.empty()) {
