...
block::block(const block_contents& contents)
  ...
  bool parse_next_key() {
    ...
    if (p == NULL || key_.size() < shared) {
      ...
      while (restart_index_ + 1 < num_restarts_ &&
             get_restart_point(restart_index_ + 1) < current_) {
        ++restart_index_;
      }
      return true;
    }
...
iterator* block::new_iterator(const comparator* cmp) {
  if (size_ < sizeof(uint32_t)) {
    return new_error_iterator(status::corruption("bad block contents"));
  }
  const uint32_t num_restarts = num_restarts();
  if (num_restarts == 0) {
