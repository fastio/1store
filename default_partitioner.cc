#include "default_partitioner.hh"
...
inline
unsigned
partitioner::zero_based_shard_of(uint64_t token, unsigned shards, unsigned sharding_ignore_msb_bits) {
...
std::vector<uint64_t>
partitioner::init_zero_based_shard_start(unsigned shards, unsigned sharding_ignore_msb_bits) {
...
token
partitioner::get_token(bytes_view key) {
    if (key.empty()) {
...
token
partitioner::get_token(uint64_t value) const {
...
uint64_t
partitioner::unbias(const token& t) const {
    return uint64_t(long_token(t)) + uint64_t(std::numeric_limits<int64_t>::min());
}

token
partitioner::bias(uint64_t n) const {
    return get_token(n - uint64_t(std::numeric_limits<int64_t>::min()));
}

sstring partitioner::to_sstring(const token& t) const {
    return seastar::to_sstring<sstring>(long_token(t));
}

dht::token partitioner::from_sstring(const sstring& t) const {
...
dht::token partitioner::from_bytes(bytes_view bytes) const {
...
int partitioner::tri_compare(const token& t1, const token& t2) const {
...
token partitioner::midpoint(const token& t1, const token& t2) const {
    auto l1 = long_token(t1);
    auto l2 = long_token(t2);
    int64_t mid;
    if (l1 <= l2) {
...
unsigned
partitioner::shard_of(const token& t) const {
    switch (t._kind) {
...
token
partitioner::token_for_next_shard(const token& t, shard_id shard, unsigned spans) const {
    uint64_t n = 0;
    switch (t._kind) {
        case token::kind::before_all_keys:
            break;
